...
class ImageProcessor:
    def __init__(self) -> None:
        ...
        self.image_np = np.zeros(self.image_res,dtype=np.uint8) # The numpy array to pour the image data into

        # TODO: Subscribe on your robot's camera topic
        # NOTE: Make sure you use the provided listener for this subscription
        self.camera_subscriber = rospy.Subscriber('/camera/image', Image, self.camera_listener)
        self.mode_publisher = rospy.Publisher('/control/mode', Float64, queue_size=1)

        self.deafult_mode = Float64()
        ...
        self.model = torch.hub.load('ultralytics/yolov5', 'custom','/home/aidawm/Desktop/robotics_ws/src/autorace_track_lane/src/best_93.pt')
        # YOLO("/home/aidawm/Desktop/robotics_ws/src/autorace_track_lane/src/best_93.pt")
    ...
    def decide_mode(self,name):
        ...
        rospy.sleep(5)
        self.mode_publisher.publish(Float64())



    def detection_service_callback(self):
        # label = req.label

        # Perform object detection on the current image frame
        if self.image_np.dtype != np.uint8:
            self.image_np = self.image_np.astype(np.uint8)
            
        image_np_rgb = cv2.cvtColor(self.image_np, cv2.COLOR_BGR2RGB)
        self.results = self.model(image_np_rgb)  # Perform object detection on the current image frame
        ...
        for result in self.results.xyxy[0]:
            ...
            self.decide_mode(name)


            # detection_count = result.boxes.shape[0]

            # for i in range(detection_count):
            #     cls = int(result[i].item())
            #     name = result.names[cls]
                
                # if name == label:
            bounding_box = result[:4].cpu().numpy()
            p1 = Point()
            p2 = Point()
